...
# Sidebar - Multi-rotor & Precise Parameter Inputs
st.sidebar.header("🌀 Multi-Rotor Settings")
num_rotors = st.sidebar.number_input("Number of Rotors (i)", min_value=1, max_value=8, value=1, step=1, help="Total number of rotors in the engine")

st.sidebar.header("🔧 Engine Geometry")
R_mm = st.sidebar.number_input("Rotor Generating Radius R (mm)", min_value=50.0, max_value=300.0, value=105.0, step=1.0)
e_mm = st.sidebar.number_input("Eccentricity e (mm)", min_value=5.0, max_value=50.0, value=15.0, step=0.1)
b_mm = st.sidebar.number_input("Rotor Width b (mm)", min_value=10.0, max_value=250.0, value=80.0, step=1.0)
r_c = st.sidebar.number_input("Compression Ratio r_c (-)", min_value=3.0, max_value=20.0, value=9.6, step=0.1)
V_pocket_cc = st.sidebar.number_input("Rotor Pocket Volume (cc)", min_value=0.0, max_value=200.0, value=20.0, step=1.0)

# Sidebar - Operating Conditions
st.sidebar.header("⚙️ Operating Conditions")
N_rpm = st.sidebar.number_input("Engine Speed N (RPM)", min_value=500, max_value=15000, value=3000, step=100)
T_wall_k = st.sidebar.number_input("Wall Temperature T_wall (K)", min_value=250, max_value=800, value=420, step=10)
A_leak_mm2 = st.sidebar.number_input("Equivalent Leakage Area (mm²)", min_value=0.0, max_value=20.0, value=1.5, step=0.1)

# Sidebar - Combustion & Fuel
st.sidebar.header("🔥 Combustion & Fuel")
AFR = st.sidebar.number_input("Air-Fuel Ratio AFR (-)", min_value=8.0, max_value=30.0, value=14.7, step=0.1)
ignition_deg = st.sidebar.number_input("Ignition Spark Timing (°SA)", min_value=400.0, max_value=700.0, value=540.0, step=1.0)
comb_dur_deg = st.sidebar.number_input("Combustion Duration (°)", min_value=5.0, max_value=180.0, value=50.0, step=1.0)
...
# Scale multi-rotor metrics
total_displacement_cc = 2.0 * (sim.V_d * 1e6) * num_rotors
total_power_kW = summary['indicated_power_kW'] * num_rotors
...
with tab_dash:
    # Key Performance Indicator (KPI) Cards (6 columns)
    col1, col2, col3, col4, col5, col6 = st.columns(6)
    col1.metric("Total Indicated Power", f"{total_power_kW:.2f} kW", help=f"Total indicated power generated by {num_rotors} rotor(s) (3 active chambers per rotor)")
    col2.metric("Engine Displacement", f"{total_displacement_cc:.1f} cc", help=f"Total equivalent engine displacement: 2 * V_d * {num_rotors} rotor(s)")
    col3.metric("Thermal Efficiency", f"{summary['indicated_efficiency_pct']:.2f} %")
    col4.metric("IMEP", f"{summary['imep_bar']:.2f} bar", help="Indicated Mean Effective Pressure (same per chamber)")
    col5.metric("Volumetric Efficiency", f"{summary['volumetric_efficiency_pct']:.2f} %")
    col6.metric("Blow-by Leakage Loss", f"{summary['leakage_loss_pct']:.2f} %")
    ...
    # 2x2 multiplots
    st.subheader("In-Cylinder Parameter Variations vs. Crank Angle (Per Chamber)")
...
with tab_theory:
    ...
    ### 2. Multi-Rotor Scaling
    In a Wankel engine, the total equivalent displacement $V_h$ is defined by Kenichi Yamamoto [24] as:
    $$V_h = 2 \\cdot V_d \\cdot i$$
    Where:
    - $V_d$ is the single-face swept volume.
    - $i$ is the number of rotors (**currently configured as {num_rotors}**).
    
    Total Indicated Power scales linearly with the number of rotors:
    $$P_{{total}} = P_{{single\\_rotor}} \\cdot i$$
